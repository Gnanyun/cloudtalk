...
# ==================== 安全配置结束 ====================
# ---------- 配置文件上传 ----------
# 优先使用 Volume（持久化），fallback 到本地 static
DATA_DIR = os.environ.get('DATA_DIR', '/data')
if not os.path.exists(DATA_DIR):
    # 本地开发环境没有 /data，用 static/uploads
    DATA_DIR = os.path.join('static', 'uploads')
...
def init_db():
    ...
    if c.fetchone()[0] == 0:
        default_admin_pass = os.environ.get('ADMIN_DEFAULT_PASS', 'admin123')
    ...
    # 老用户：用 username 填充 display_name
...
# ---------- 帖子和评论作者信息填充 ----------
def enrich_author_info(item_dict, username_field='author'):
    """给帖子/评论字典补充作者 id、display_name、avatar"""
    username = item_dict.get(username_field, '')
    if not username:
        item_dict['author_id'] = None
        item_dict['author_display'] = '匿名'
        item_dict['author_avatar'] = '/api/default-avatar?name=匿名'
        return item_dict
    
    conn = get_db()
    author = conn.execute('SELECT id, avatar, display_name FROM users WHERE username = ?', (username,)).fetchone()
    conn.close()
    
    if author:
        item_dict['author_id'] = author['id']
        item_dict['author_display'] = author['display_name'] or username
        item_dict['author_avatar'] = author['avatar'] or f'/api/default-avatar?name={username}'
    else:
        item_dict['author_id'] = None
...
def register():
    ...
    # 用户名长度校验
    ok, msg = validate_length(username, 2, 20)
    if not ok:
        return jsonify({'status': 'error', 'msg': '用户名' + msg}), 400

    # 密码强度校验
    if not password or len(password) < 8:
        return jsonify({'status': 'error', 'msg': '密码长度不能少于 8 位'}), 400
    if len(password) > 72:
        return jsonify({'status': 'error', 'msg': '密码长度不能超过 72 位'}), 400
...
def login():
    ...
    if not check_rate_limit(client_ip):
        return jsonify({'status': 'error', 'msg': '尝试过于频繁，请 5 分钟后再试'}), 429
    ...
    if not user or not check_password_hash(user['password'], password):
        record_attempt(client_ip)  # 记录失败
        return jsonify({'status': 'error', 'msg': '用户名或密码错误'}), 401
    
    clear_attempts(client_ip)  # 登录成功清空记录
...
def upload_avatar():
    ...
    if not verify_image(filepath):
        os.remove(filepath)
        return jsonify({'status': 'error', 'msg': '文件不是合法的图片'}), 400
...
def get_topic(topic_id):
    ...
    # 如果帖子被隐藏，且当前访问者不是作者本人，返回 404
    if topic_dict.get('hidden') == 1:
        if session.get('username') != topic_dict['author']:
            conn.close()
            return jsonify({'status': 'error', 'msg': '话题不存在'}), 404
    ...
    # 获取作者信息（头像、id、显示名）
    author = conn.execute('SELECT id, avatar, display_name FROM users WHERE username = ?', 
                          (topic_dict['author'],)).fetchone()
    if author:
        topic_dict['author_id'] = author['id']
        topic_dict['author_display'] = author['display_name'] or topic_dict['author']
        topic_dict['author_avatar'] = author['avatar'] or f'/api/default-avatar?name={topic_dict["author"]}'
    else:
        topic_dict['author_id'] = None
        topic_dict['author_display'] = topic_dict['author']
...
def get_comments(topic_id):
    ...
    for c in comments:
        ...
        # 补充评论者信息
        user = conn.execute('SELECT id, avatar, display_name FROM users WHERE username = ?', 
                            (comment['author'],)).fetchone()
        if user:
            comment['author_id'] = user['id']
            comment['display_name'] = user['display_name'] or comment['author']
            comment['avatar'] = user['avatar'] or f'/api/default-avatar?name={comment["author"]}'
        else:
...
def add_comment(topic_id):
    ...
    if not ok:
        return jsonify({'status': 'error', 'msg': '评论' + msg}), 400
...
def get_user_tab_content(username, tab):
    ...
    # 其余分支不变...
        ...
        for f in follows:
            user = conn.execute('SELECT id, avatar, display_name FROM users WHERE username = ?', (f['username'],)).fetchone()
            result.append({
                'username': f['username'],
                'user_id': user['id'] if user else None,
                'display_name': (user['display_name'] if user and user['display_name'] else f['username']),
...
def get_categories():
    # 已登录时用 session 用户名，未登录时只用 system
...
def add_category():
    ...
    # 分类名长度校验
    ok, msg = validate_length(name, 1, 20)
    if not ok:
        return jsonify({'status': 'error', 'msg': '分类名' + msg}), 400
...
def create_post():
    ...
    # 标题长度校验
    ok, msg = validate_length(title, 1, 100)
    if not ok:
        return jsonify({'status': 'error', 'msg': '标题' + msg}), 400

    # 内容长度校验
    ok, msg = validate_length(content, 1, 5000)
    if not ok:
        return jsonify({'status': 'error', 'msg': '内容' + msg}), 400
...
def get_checkin_status(username):
    if username != session['username']:
        return jsonify({'status': 'error', 'msg': '只能查看自己的签到状态'}), 403
    from datetime import date, timedelta
    today = date.today()
    today_str = today.isoformat()
...
def get_user_flowers(username):
    if username != session['username']:
        return jsonify({'status': 'error', 'msg': '只能查看自己的花朵'}), 403
    conn = get_db()
...
def update_user_profile(username):
    if username != session['username']:
        return jsonify({'status': 'error', 'msg': '只能修改自己的资料'}), 403
    ...
    # 修改显示名
    if display_name:
        ok, msg = validate_length(display_name, 1, 20)
        if not ok:
            conn.close()
            return jsonify({'status': 'error', 'msg': '昵称' + msg}), 400
...
def get_user_profile(username):
    ...
    user = conn.execute('SELECT id, username, bio, avatar, display_name FROM users WHERE username = ?', (username,)).fetchone()
    if not user:
        conn.close()
        return jsonify({'status': 'error', 'msg': '用户不存在'}), 404
    bio = user['bio'] if user['bio'] else '这个人很懒，什么都没写~'
...
# ---------- 按 ID 查用户（点击头像跳转用） ----------
@app.route('/api/user/by-id/<int:user_id>', methods=['GET'])
def get_user_by_id(user_id):
    conn = get_db()
    user = conn.execute('SELECT id, username, display_name, avatar FROM users WHERE id = ?', (user_id,)).fetchone()
    conn.close()
    if not user:
        return jsonify({'status': 'error', 'msg': '用户不存在'}), 404
    return jsonify({
        ...
        'user': {
            ...
            'username': user['username'],
            'display_name': user['display_name'] or user['username'],
            'avatar': user['avatar'] or f'/api/default-avatar?name={user["username"]}'
        }
    })
...
def admin_login():
    ...
    # 管理员登录更严格：3 次 / 5 分钟
    if not check_rate_limit(client_ip, max_attempts=3, window=300):
        return jsonify({'status': 'error', 'msg': '尝试过于频繁，请 5 分钟后再试'}), 429
...
def create_group():
    ...
    # 群组名校验
    ok, msg = validate_length(name, 1, 50)
    if not ok:
        return jsonify({'status': 'error', 'msg': '群组名' + msg}), 400
...
def get_group_detail(group_id):
    ...
    members = conn.execute('''
        SELECT gm.username, gm.role, gm.joined_at, u.avatar, u.id as user_id, u.display_name
...
def send_group_chat(group_id):
    ...
    ok, msg = validate_length(content, 1, 500)
    if not ok:
        return jsonify({'status': 'error', 'msg': '消息' + msg}), 400
